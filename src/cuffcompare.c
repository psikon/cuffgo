...
SEXP parseCuffcompare(SEXP filename) {
	
	//FILE *cuffcompare = fopen(CHAR(STRING_ELT(filename, 0)), "r");
	FILE *cuffcompare = fopen(CHAR(STRING_ELT(filename, 0)), "r");
	...
	if (cuffcompare) {
		while ((endLine = fgetc(cuffcompare)) != EOF) {
			if (endLine == '\n') {
				numLines++;
			}
		}
	...
	SEXP chr, program, feature, start, end, score, strand, frame, gene_id,
			transcript_id, exon_number, gene_name, oId, nearest_ref, 
			class_code, tss_id, list, list_names;
	...
	char *names[16] = { "chr", "program", "feature", "start", "end", "score",
			"strand", "frame", "gene_id", "transcript_id", "exon_number",
			"gene_name", "oId", "nearest_ref", "class_code", "tss_id" };
	// open the file for reading only
	cuffcompare = fopen(CHAR(STRING_ELT(filename, 0)), "r");
	// read the line until the end of file
	while (fgets(line, sizeof line, cuffcompare)) {
...
// init the names of the vectors
	PROTECT(list_names = allocVector(STRSXP, 16));
...
// build a list of the vectors
	...
	SET_VECTOR_ELT(list, 13, nearest_ref);
	SET_VECTOR_ELT(list, 14, class_code);
	SET_VECTOR_ELT(list, 15, tss_id);
